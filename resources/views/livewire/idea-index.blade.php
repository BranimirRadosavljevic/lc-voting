        ...
        <div class="mt-8">
            @if ($hasVoted)
                <button class="w-20 bg-blue border border-blue hover:bg-blue-hover 
                    font-bold text-white text-xxs uppercase rounded-xl transition duration-150 ease-in px-4 py-3 ">Voted</button>           
            @else
        ...
        <div class="w-full flex flex-col justify-between mx-2 md:mx-4">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between mt-6">
                ...
                <div x-data="{isOpen: false}" class="flex items-center space-x-2 mt-4 md:mt-0">
                    ...
                    text-center w-28 h-7 px-4 py-2">{{ $idea->status->name }}</div>
                    <button @click="isOpen=!isOpen"
                        ...
                        <ul x-cloak x-show.transition.origin.top.left="isOpen" @click.away="isOpen=false"
                            @keydown.escape.window="isOpen=false" class="absolute w-44 text-left font-semibold bg-white shadow-dialog rounded-xl py-3 md:ml-8
